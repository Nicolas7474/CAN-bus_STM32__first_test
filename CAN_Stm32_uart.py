...
import GUI_CAN_Stm32_uart  # Import our second file
...
class STM32Monitor:
    ...
    def __init__(self):   
        ...
        self.max_rows = 16   # Prevent infinite growth
        self.can_speed_packet = None

        # Load resources from support.py
        self.fonts = GUI_CAN_Stm32_uart.load_assets()
        self.t_off, self.t_on, self.t_stop = GUI_CAN_Stm32_uart.create_button_themes()
        self.table_theme = GUI_CAN_Stm32_uart.theme_table()
        self.error_theme = GUI_CAN_Stm32_uart.theme_error()
        ...
        with dpg.window(tag="primary_window", label="STM32F469"):
            ...
            with dpg.group(horizontal=True):     
                ...
                def get_port_list():  # list available devices (remove the redondant COMx from the device's description and sort by COM growing number)  
                    ...
                    # Logic: "COM17 - Desc" -> "17 - Desc" -> "17" -> 17
                    list.sort(key=lambda x: int(re.search(r'COM(\d+)', x).group(1)) if "COM" in x else 0)                             
                    return list
                def update_ports():
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.group(width=130, height=20):                      
                    ...
                    dpg.add_text("Discarded frames", color=[245, 255, 200])
                    with dpg.group(horizontal=True):
            ...
            with dpg.group(horizontal=True):               
                ...
                start_btn = dpg.add_button(
                    ...
                    user_data=[self, "cercle", "port_dropdown", "can_speed_dropdown"], callback=GUI_CAN_Stm32_uart.toggle_start_handler)   
                ...
                dpg.add_spacer(width=15)              
                # Send button
                send_btn = dpg.add_button(label="Send", width=60, height=26, callback=self.send_command)
        ...
        with dpg.window(tag="error_modal", label="Error", 
            ...
            dpg.add_text("Check connection.", tag="error_messagechk_text", color=(255, 255, 235)),
                    ...
                    if data_in:
                        ...
                        while b'\x00' in buffer:
                            ...
                            try:                                
                                ...
                                # Now you can use your fixed indices again because it's decoded!                             
                                # decoded_msg[0:2] is StdId
                                # decoded_msg[2] is RTR
                                # decoded_msg[3] is DLC
                                # decoded_msg[4:4+dlc] is Data                             
                                # using int.from_bytes and simple list slicing (above), Python will handle this extremely quickly                              
                                can_id = int.from_bytes(decoded_msg[0:2], byteorder='little')
                                rtr = decoded_msg[2]
                                dlc = decoded_msg[3]
                                payload = decoded_msg[4:4+dlc]                               
    ...
    def process_queue(self):
        ...
        while not self.rx_queue.empty() and processed < max_per_frame:
            ...
            with dpg.table_row(parent="can_table_disp"):
            ...
            rows = dpg.get_item_children("can_table_disp", 1)
